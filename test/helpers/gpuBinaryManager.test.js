...
// divergent semantics its wrappers preserve (CUDA throws on cancel, llama
// returns { cancelled: true }). Runs outside Electron: stub electron and the
// download/extract layer (covered by extractArchive.test.js) before loading.
...
// Asset configs are keyed by `${platform}-${arch}` — pin to linux-x64 so the
// suite behaves identically on any host.
...
function makeRelease(assetName, overrides = {}) {
  return {
    ...
    assets: [
      ...
      { name: assetName, browser_download_url: `https://dl/${assetName}`, size: 1000, ...overrides },
    ],
  };
}
