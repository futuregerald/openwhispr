...
// Shared download/install pipeline for optional GPU server binaries fetched
// from GitHub releases (whisper CUDA, whisper Vulkan, llama Vulkan). Backends
// subclass with a config describing their release and per-platform assets:
//   name             label used in errors and logs
//   releaseUrl       GitHub API release URL (pinned tag or latest)
//   expectedDigests  optional in-app pinned sha256 hex per asset name; when
//                    absent, the digest reported by the GitHub API is used
//   assets           keyed by `${platform}-${arch}`:
//     assetName        exact release asset name (or assetPattern)
//     assetPattern     regex matched against asset names (or assetName)
//     binaryName       server binary filename inside the archive
//     outputName       filename installed into userData/bin
//     libPattern       optional regex for companion libs copied beside it
class GpuBinaryManager {
  ...
  // Downloaded binaries are unsigned and release assets are mutable — fail
  // closed when the archive doesn't match the pinned (or API-reported) digest.
