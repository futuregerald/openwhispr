...
if (!swiftTarget) {
  console.error(`[call-detector] Unsupported architecture: ${targetArch}`);
  process.exit(1);
}

const projectRoot = path.resolve(__dirname, "..");
const swiftSource = path.join(projectRoot, "resources", "macos-call-detector.swift");
const outputDir = path.join(projectRoot, "resources", "bin");
const outputBinary = path.join(outputDir, "macos-call-detector");
const hashFile = path.join(outputDir, `.macos-call-detector.${targetArch}.hash`);
...
function log(message) {
  console.log(`[call-detector] ${message}`);
}
...
if (!fs.existsSync(swiftSource)) {
  console.error(`[call-detector] Swift source not found at ${swiftSource}`);
  process.exit(1);
}
...
if (result.status !== 0) {
  console.error("[call-detector] Failed to compile macOS call detector binary.");
...
try {
  ...
  console.warn(`[call-detector] Unable to set executable permissions: ${error.message}`);
}

if (!verifyBinaryArch(outputBinary, targetArch)) {
  console.error(
    `[call-detector] FATAL: Compiled binary architecture does not match target (${targetArch}).`
  );
  process.exit(1);
}
...
log(`Successfully built macOS call detector binary (${targetArch}).`);
